...
class Config:
    ...
    SYSTEM_RUNTIME_NOTIFICATION_ENABLED = _load_bool_env(
        "SYSTEM_RUNTIME_NOTIFICATION_ENABLED", False
    )
    SYSTEM_RUNTIME_NOTIFICATION_WEBHOOK_URL = os.environ.get(
        "SYSTEM_RUNTIME_NOTIFICATION_WEBHOOK_URL", ""
    ).strip()
    SYSTEM_RUNTIME_NOTIFICATION_WEBHOOK_PREFIX = os.environ.get(
        "SYSTEM_RUNTIME_NOTIFICATION_WEBHOOK_PREFIX", "[营养监测系统运行]"
    ).strip()
    SYSTEM_RUNTIME_NOTIFICATION_TIME = os.environ.get(
    ...
    VIDEO_DOWNLOAD_MAX_IN_FLIGHT = max(1, _load_int_env("VIDEO_DOWNLOAD_MAX_IN_FLIGHT", 4))
    VIDEO_EXTRACT_MAX_IN_FLIGHT = max(1, _load_int_env("VIDEO_EXTRACT_MAX_IN_FLIGHT", 4))
    VIDEO_RECORDING_DISPATCH_LEASE_SECONDS = max(
        60,
        _load_int_env("VIDEO_RECORDING_DISPATCH_LEASE_SECONDS", 300),
    )
    VIDEO_RECORDING_QUEUED_LEASE_SECONDS = max(
        3600,
        _load_int_env("VIDEO_RECORDING_QUEUED_LEASE_SECONDS", 21600),
    )
    VIDEO_RECORDING_HEARTBEAT_LEASE_SECONDS = max(
        600,
        _load_int_env("VIDEO_RECORDING_HEARTBEAT_LEASE_SECONDS", 1800),
    )
    VIDEO_RECORDING_DISPATCH_MAX_ATTEMPTS = max(
        1,
        _load_int_env("VIDEO_RECORDING_DISPATCH_MAX_ATTEMPTS", 20),
    )
    ...
    VIDEO_EXTRACT_MAX_RUNTIME_SECONDS = _load_int_env("VIDEO_EXTRACT_MAX_RUNTIME_SECONDS", 7200)
    VIDEO_EXTRACT_FFMPEG_TIMEOUT_SECONDS = _load_int_env("VIDEO_EXTRACT_FFMPEG_TIMEOUT_SECONDS", 1800)
    VIDEO_EXTRACT_CPU_THREADS_PER_JOB = _load_int_env("VIDEO_EXTRACT_CPU_THREADS_PER_JOB", 2)
    ...
    VIDEO_EXTRACT_MIN_DECODE_COMPLETION_RATIO = _load_float_env("VIDEO_EXTRACT_MIN_DECODE_COMPLETION_RATIO", 0.99)
    ...
    LEGACY_ANALYSIS_MAX_WIDTH = _load_int_env("LEGACY_ANALYSIS_MAX_WIDTH", 960)
    LEGACY_ANALYSIS_MAX_HEIGHT = _load_int_env("LEGACY_ANALYSIS_MAX_HEIGHT", 540)
