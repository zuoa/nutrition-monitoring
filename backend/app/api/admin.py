...
def get_config():
    ...
    return api_ok({
        ...
        "legacy_analysis_max_width": cfg.get("LEGACY_ANALYSIS_MAX_WIDTH", 960),
        "legacy_analysis_max_height": cfg.get("LEGACY_ANALYSIS_MAX_HEIGHT", 540),
        "legacy_quick_stable_frames_min": cfg.get("LEGACY_QUICK_STABLE_FRAMES_MIN", 1),
        "legacy_min_event_gap_seconds": cfg.get("LEGACY_MIN_EVENT_GAP_SECONDS", 0.8),
        ...
        **sample_stats,
        ...
        "sample_image_min_edge": cfg.get("SAMPLE_IMAGE_MIN_EDGE", 128),
        "sample_image_max_pixels": cfg.get("SAMPLE_IMAGE_MAX_PIXELS", 16_777_216),
        "sample_image_max_aspect_ratio": cfg.get("SAMPLE_IMAGE_MAX_ASPECT_RATIO", 3.0),
...
@bp.route("/video-sources/hikvision/discover", methods=["POST"])
@role_required("admin")
def discover_hikvision_video_source():
    data = request.get_json() or {}
    source = None
    ...
    try:
        payload = _video_source_manager().discover_hikvision_device(data, existing_source=source)
    except VideoSourceConfigError as e:
        return api_error(str(e))
    return api_ok(payload)
