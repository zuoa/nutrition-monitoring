...
def _normalize_hikvision_config(
    ...
    if not isinstance(cameras, list) or not cameras:
        raise VideoSourceConfigError("cameras 不能为空")

    existing_by_channel = _credentials_by_channel(existing_credentials)
    ...
    for item in cameras:
        ...
        existing_camera_credentials = existing_by_channel.get(channel_id, {})
        username = _as_optional_string(item.get("username")) or _as_optional_string(existing_camera_credentials.get("username"))
        password = _as_optional_string(item.get("password")) or _as_optional_string(existing_camera_credentials.get("password"))
        if not username:
            raise VideoSourceConfigError(f"摄像头 {channel_id} 的 username 不能为空")
        if not password:
            raise VideoSourceConfigError(f"摄像头 {channel_id} 的 password 不能为空")
    ...
    return ({"cameras": normalized_cameras}, {"cameras": normalized_credentials})
